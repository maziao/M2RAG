...
class LMDeployMixin(BaseModelMixin):
    def __init__(
        self,
        model: Optional[str],
        tokenizer_path: Optional[str],
        lmdeploy_api_key: Optional[str],
        lmdeploy_service_url: Optional[str]
    ) -> None:
        if lmdeploy_api_key is None:
            lmdeploy_api_key = os.environ.get('LMDEPLOY_API_KEY', 'pseudo-api-key')
        
        if lmdeploy_service_url is None:
            lmdeploy_service_url = os.environ.get('LMDEPLOY_SERVICE_URL', None)
        assert lmdeploy_service_url is not None, f"`LMDEPLOY_SERVICE_URL` is not set"
        
        self.api_key = lmdeploy_api_key
        self.base_url = lmdeploy_service_url
        self.client = OpenAI(api_key=lmdeploy_api_key, base_url=lmdeploy_service_url)
...
class LMDeployLLM(LMDeployMixin, LLM):
    def __init__(
        self,
        model: Optional[str],
        tokenizer_path: Optional[str],
        ...
        super().__init__(model=model, tokenizer_path=tokenizer_path, lmdeploy_api_key=lmdeploy_api_key, lmdeploy_service_url=lmdeploy_service_url)
        super(LMDeployMixin, self).__init__(name=f"lmdeploy-{self.model}")
...
@MODEL.register_module
class LMDeployVLM(LMDeployMixin, VLM):
    def __init__(
        self,
        model: Optional[str],
        tokenizer_path: Optional[str],
        ...
        super().__init__(model=model, tokenizer_path=tokenizer_path, lmdeploy_api_key=lmdeploy_api_key, lmdeploy_service_url=lmdeploy_service_url)
        super(LMDeployMixin, self).__init__(
            name=f"lmdeploy-{self.model}",
